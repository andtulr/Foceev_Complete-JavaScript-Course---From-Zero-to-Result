...
2. - Создайте функцию, которая будет принимать в себя 1 параметр.___ //function fibonacciGenerator (n) { 
...
3. Результат вашей последовательности запишите в пустую переменную - массив, например output = []___ //let output = []
...
function fibonacciGenerator (n) {
    ...
    if (n === 1) {
        ...
        output = [0, 1]
        for (let i = 2; i < n; i++) {
            output.push(output.at(-2) + output.at(-1)) //output.push(output[output.length - 2] + output[output.length - 1]);   
        }
    }
    return output
}
let res = fibonacciGenerator(5)
console.log(res) // (5) - [ 0, 1, 1, 2, 3 ] иТД   
